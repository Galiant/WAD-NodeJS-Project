...
app.get('/createspecial', function(req, res) {
  let sql = 'CREATE TABLE offers (Id int NOT NULL AUTO_INCREMENT PRIMARY KEY, Name varchar(255), Brand varchar(255), Category varchar(255), Image varchar(255), Price decimal(8,2), Offer decimal(8,2));';
  let query = db.query(sql, (err, res) => {
    if(err) throw err;
    console.log(res);
  });
...
// ************* SPECIAL OFFER **********
// SQL Insert Data Example
app.get('/insertoffer', function(req, res) {
  let sql = 'INSERT INTO offers (Name, Brand, Category, Image, Price, Offer) VALUES ("OhLaLa Stroller Black Night", "Chicco", "Pushchairs & Strollers", "chicco.jpg", 99.99, 84.99)';
  let query = db.query(sql, (err, res) => {
    if(err) throw err;
    console.log(res);
  });
...
app.get('/offers', function(req, res){
  ...
  let query = db.query(sql, (err, res1) => {
    if(err) throw(err);
    res.render('offers', {root: VIEWS, res1}); // use the render command so that response object renders a HTML page
  });
...
app.get('/offeritem/:id', function(req, res){
  ...
  let query = db.query(sql, (err, res1) =>{
    if(err)
    throw(err);
    res.render('offeritem', {root: VIEWS, res1}); // use the render command so that the response object renders a HHTML page
  });
...
app.post('/createoffer', function(req, res){
  var name = req.body.name;
  let sql = 'INSERT INTO offers (Name, Brand, Category, Image, Price, Offer) VALUES ("'+name+'", "'+req.body.brand+'", "'+req.body.category+'", "'+req.body.image+'", '+req.body.price+', '+req.body.offer+')';
  let query = db.query(sql, (err, res) => {
    if(err) throw err;
    console.log(res);
    console.log("The name of the product is " + name);
...
app.post('/editoffer/:id', function(req, res) {
  let sql = 'UPDATE offers SET Name = "'+req.body.newname+'", Brand = "'+req.body.newbrand+'", Category = "'+req.body.newcategory+'", Image = "'+req.body.newimage+'", Price = '+req.body.newprice+', Offer = '+req.body.newoffer+' WHERE Id = '+req.params.id+'';
  let query = db.query(sql, (err, res) => {
    if(err)
    // throw(err);
    console.log("Oops... Something went wrong....");
  ...
  res.redirect("/offeritem/" + req.params.id);
});
